...
class HistoryPayloadTranslator:
    def history_payload_item(self, entry: QuizHistoryEntry) -> HistoryPayloadItem:
        ...
        payload_scored_performance = HistoryScoredPerformance(
            quiz_performance,
            score_value,
        )
        return HistoryPayloadItem(played_at, payload_scored_performance)

    def history_entry(self, payload_item: HistoryPayloadItem) -> QuizHistoryEntry:
    ...
    def history_payload_item_from_dictionary(
        self,
    ...
    def history_payload_dictionary(self, payload_item: HistoryPayloadItem) -> object:
...
class StatePayloadReader:
    def __init__(
        self,
        quiz_mapper: QuizPayloadMapper,
        history_payload_translator: HistoryPayloadTranslator,
    ) -> None:
        self.quiz_mapper = quiz_mapper
        self.history_payload_translator = history_payload_translator

    def state_payload(self, payload_source: object) -> StatePayload:
    ...
    def game_snapshot(self, state_payload: StatePayload) -> GameSnapshot:
        quiz_mapper = self.quiz_mapper
        history_payload_translator = self.history_payload_translator
        quiz_payload_items = state_payload.quiz_payload_items
        quizzes = tuple(
            quiz_mapper.from_payload_item(payload_item)
            for payload_item in quiz_payload_items.values
        )
        payload_content = state_payload.content
        history_payload_items = payload_content.history_payload_items
        history_entries = tuple(
            history_payload_translator.history_entry(payload_item)
            for payload_item in history_payload_items.values
        )
...
class StatePayloadWriter:
    ...
    def payload_dictionary(self, state_payload: StatePayload) -> object:
        quiz_mapper = self.quiz_mapper
        quiz_payload_items = state_payload.quiz_payload_items
        payload_content = state_payload.content
        ...
        return {
            ...
            constants.STATE_KEY_HISTORY: [
                self.history_payload_translator.history_payload_dictionary(payload_item)
                for payload_item in history_payloads.values
            ],
        }

    def state_payload(self, game_snapshot: GameSnapshot) -> StatePayload:
        quiz_mapper = self.quiz_mapper
        history_payload_translator = self.history_payload_translator
        ...
        history_payload_items = tuple(
            history_payload_translator.history_payload_item(entry)
            for entry in game_record_book.play_history
        )
        payload_content = StatePayloadContent(
...
class StatePayloadMapper:
    def __init__(self, quiz_mapper: Optional[QuizPayloadMapper] = None) -> None:
        resolved_quiz_mapper = quiz_mapper or QuizPayloadMapper()
        history_payload_translator = HistoryPayloadTranslator()
        self.state_payload_reader = StatePayloadReader(
            resolved_quiz_mapper,
            history_payload_translator,
        )
        self.state_payload_writer = StatePayloadWriter(
            resolved_quiz_mapper,
            history_payload_translator,
        )

    def to_state_payload(self, game_snapshot: GameSnapshot) -> StatePayload:
        state_payload_writer = self.state_payload_writer
        return state_payload_writer.state_payload(game_snapshot)

    def from_state_payload(self, state_payload: StatePayload) -> GameSnapshot:
        state_payload_reader = self.state_payload_reader
        return state_payload_reader.game_snapshot(state_payload)

    def _state_payload(self, payload_source: object) -> StatePayload:
        return self.state_payload_reader.state_payload(payload_source)

    def _payload_dictionary(self, state_payload: StatePayload) -> object:
        return self.state_payload_writer.payload_dictionary(state_payload)
