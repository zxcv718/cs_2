...
클래스와 객체의 개념도 퀴즈 게임 구조 안에서 자연스럽게 적용했습니다. 최근 리팩토링에서는 단일 책임 원칙을 더 잘 맞추기 위해 `Quiz`와 `QuizGame`의 책임을 다시 나누었습니다. `Quiz`는 이제 문제 한 개의 도메인 동작만 표현하고, 입력 검증과 정규화는 `QuizFactory`와 `quiz_components` 값 객체들이 담당합니다. `QuizGame`은 메인 루프를 돌며 서비스를 조율하는 진입점만 맡고, 세부 책임은 더 작은 서비스로 분리했습니다. 예를 들어 기본 상태 생성은 `DefaultGameStateFactory`, 상태 복구는 `GameBootstrapService`, 상태 저장은 `GamePersistenceService`, 메뉴 분기는 `MenuActionDispatcher`, 실제 퀴즈 진행은 `QuizSessionService`, 점수 계산은 `QuizScoringService`, 최고 점수 비교와 history 반영은 `QuizResultRecorder`, 종료 처리와 중단 저장은 `GameShutdownService`가 담당합니다. 이때 `__init__` 메서드는 객체가 처음 만들어질 때 필요한 협력 객체를 준비하는 데 사용했고, `self`는 각 객체 자신의 속성과 메서드에 접근하기 위해 사용했습니다. 이렇게 속성은 데이터 보관, 메서드는 동작 수행이라는 역할로 나누어 객체지향 구조를 구성했습니다.
...
- `GameRuntimeState`: 현재 메모리 안의 퀴즈, 최고 점수, history 상태를 묶습니다.
